...
@register_base_model
class LlamaCpuInferenceModel(LlamaPretrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        #根据config.quant_type 转换成 cpu对应的权重 目前仅支持fp16/bf16（较好的cpu上）
        #后续支持wint8/w8a8 等
    ...
    def set_transformer_block(self, transformer_config):
        self.transformer_block = FusedMultiTransformerCPU(transformer_config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids.squeeze(axis=1))
...
class LlamaForCausalLMCpuInferenceModel(GenerationCpuInferenceModel, LlamaPretrainedModel):

    _keys_to_ignore_on_load_missing = [r"lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.llama = LlamaCpuInferenceModel(config)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        past_output_len=seq_len_decoder-seq_len_encoder
        past_seq_len = 0 if past_output_len==0 else seq_len_decoder-past_output_len-1
        #decoder
        if past_seq_len!=0:
